...
export default function TenantsPage() {
    ...
    const { data, error } = await supabase
      ...
      .select(`
        *,
        lease_tenants(
          is_primary,
          lease:leases(id, status, monthly_rent, start_date, end_date, property_id, property:properties(name))
        )
      `)
      .eq("household_id", householdId)
      .is("deleted_at", null)
      .order("created_at", { ascending: false });
    if (!error) setTenants((data ?? []) as TenantWithLeases[]);
    setLoading(false);
  };
  ...
  const handleDelete = async () => {
    ...
    if (hasActiveLeases) {
      toast.error("该租客还有生效中的租约，无法删除。请先归档相关租约。");
      setDeletingId(null);
      return;
    }
    ...
    <div className="p-4 md:p-6 max-w-4xl">
      ...
      {tenants.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {tenants.map((tenant) => {
            const activeLease = tenant.lease_tenants
              ?.map((lt) => lt.lease)
              .find((l) => l && l.status === "active") ?? null;
            const archivedCount = tenant.lease_tenants
              ?.filter((lt) => lt.lease && lt.lease.status !== "active").length ?? 0;
            return (
              <Card key={tenant.id} className="hover:border-primary/30 transition-colors">
                <CardContent className="pt-4">
                  ...
                  {activeLease ? (
                    <Link
                      ...
                      className="block rounded-lg bg-primary-soft/40 hover:bg-primary-soft/70 transition-colors px-3 py-2 mb-2"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            至 {formatDate(activeLease.end_date)}
                          </p>
                        </div>
                        <span className="num text-sm font-semibold text-primary shrink-0">
                    ...
                    <Link
                      href="/leases"
                      className="block rounded-lg border border-dashed border-border px-3 py-2 mb-2 text-center text-xs text-muted-foreground hover:bg-muted/40 transition-colors"
                    >
                      暂无生效租约 · 点击去创建
                    </Link>
                  )}
                  ...
                  {archivedCount > 0 && (
                    <p className="text-[11px] text-muted-faint mb-1">
                      历史租约 {archivedCount} 份
                    </p>
                  )}
